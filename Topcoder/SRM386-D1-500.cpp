/*
we should devide the set of points into non overlapping subsets of size >= 3 (sets of size 3 can overlap with any other set)
foreach set its cost is the area of the convux hull containig its points
find the minimum cost of dp[2^n - 1] using a well known dp with 3^n complexity
*/
...
public:
    ...
    double getArea(vector <int> x, vector <int> y)
    {
        ...
        for(int i=1;i<(1<<n);i++)
        {
            ...
            vector<point>hull;
            convexHull(v,hull);
            long double area = polygonArea(hull);
            cover[i] = area;
        }
        dp[0] = 0;
        for(int i=1;i<(1<<n);i++)
        ...
        for(int i=0;i<(1<<n);i++)
        {
            int i_cnt = __builtin_popcount(i);
            if( i_cnt< 3 && i_cnt != 0)
                continue;
//            dp[i] = inf;
            int msk = (1<<n) - i;
            ...
            {
                int j_cnt = __builtin_popcount(j);
                if(j_cnt < 3 || (j_cnt != i_cnt && i_cnt - j_cnt < 3))
                    continue;
                dp[i|j] = min(dp[i|j], cover[j] + dp[i]);
            }
            for(int j :threes)
            {
                dp[i|j] = min(dp[i|j], cover[j] + dp[i]);
